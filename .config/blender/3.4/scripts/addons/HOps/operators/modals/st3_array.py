...
class HOPS_OT_ST3_Array(bpy.types.Operator):
    ...
    def setup_objects(self, context):
        ...
        for obj_data in self.mod_controller.validated_obj_datas():
            ...
            if obj_data.active_mod_data().was_created and len(obj_data.mod_datas) > 1:
                ...
                curr_mod = obj_data.mod_datas[-1].mod
                ...
                cur_constant_offset = mod_constant_offset_object(prev_mod)
                cur_relative_offset = mod_relative_offset_object(prev_mod)
    ...
    def add_an_array(self):
        self.mod_controller.create_new_mod(mod_type='ARRAY')
        for mod in self.mod_controller.active_modifiers():
    ...
    def array_to_empty_driver(self, context):
        ...
        # Build main empty controller
        # valid = False
        # for key, item in obj_empty_data.items():
        #     if len(item) > 1:
        #         valid = True
        #         break
        
        # if len(obj_empty_data) > 1:
        #     valid = True

        # if not valid: return

        # main_obj = self.mod_controller.active_obj
        # if main_obj:
        #     self.assign_main_empty(context, main_obj, obj_empty_data)
