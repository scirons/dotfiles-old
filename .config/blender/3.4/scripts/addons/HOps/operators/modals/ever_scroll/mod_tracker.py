...
class Mod_Tracker:
    ...
    def make_selected_active(self, index=0, reveal_object=False):
        if index < len(self.mods):
            ...
            if not reveal_object: return

            if self.current_mod.type == 'NODES':
                objects = get_node_graph_objects(self.current_mod)
                for obj in objects:
                    if obj.hide_get():
                        obj.hide_set(False)
                    else:
            ...
            obj = get_mod_object(self.current_mod)
