...
from ... addon.property.preference.property import profile_dir
...
class HOPS_OT_AdjustBevelOperator(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        for object_name in self.bevel_objects:
            ...
            if event.type == "M" and event.value == "PRESS" and event.alt:
                miter_types = ["MITER_SHARP", "MITER_ARC", "MITER_PATCH"]
                modifier.miter_outer = miter_types[(miter_types.index(modifier.miter_outer) + 1) % len(miter_types)]
                self.report({'INFO'}, F'Miter Outer Type : {modifier.miter_outer}')
    ...
    def draw_ui(self, context):
        ...
        if self.master.should_build_fast_ui():
            ...
            help_items["STANDARD"] = [
                ...
                ("Alt + M",        f"Adjust the miter outer algorithm ({self.active_bevel_modifier.miter_outer})"),
                ...
                ("X",              f"Set bevel to 50% of current ({self.active_bevel_modifier.width/2:.2f})"),
